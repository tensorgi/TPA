# T6-LinearAttention using SwiGLU, RMSNorm, and Partial RoPE with Decay
...
class CPLinear(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.W_A_q = nn.Linear(self.in_features, self.n_head * self.q_rank, bias=False)
        ...
        self.W_B_q = nn.Linear(self.in_features, self.q_rank * self.head_dim, bias=False)
    ...
    def forward(self, x):
        ...
        cos, sin, decay = self.rotary(B_q)
        B_q, B_k = apply_rotary_emb(B_q, cos, sin, decay), apply_rotary_emb(B_k, cos, sin, decay)
