# Llama Linear Attention using SwiGLU, RMSNorm, and Partial RoPE with Decay
...
class RotaryDecay(torch.nn.Module):
	...
	def __init__(self, dim, base=10000, decay_base=0.999):
		super().__init__()
...
class CausalSelfAttention(nn.Module):
	...
	def __init__(self, config):
		...
		rotary_dim = int(self.head_dim * config.rope_partial_factor)
	...
	def forward(self, x):
		...
		q, k = apply_rotary_emb(q, cos, sin, decay), apply_rotary_emb(k, cos, sin, decay)
		...
		y = y.transpose(1, 2).contiguous().reshape(B, T, -1).clone()
		y = self.c_proj(y)
		return y
