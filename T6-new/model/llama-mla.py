...
class CausalSelfAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.head_dim = config.head_dim + config.qk_rope_head_dim
        ...
        self.q_a_proj = torch.nn.Linear(self.n_embd, self.q_lora_rank, bias=False)
        self.q_a_layernorm = RMSNorm(self.q_lora_rank)
        self.q_b_proj = torch.nn.Linear(self.q_lora_rank, self.n_head * self.q_head_dim, bias=False)
        self.kv_a_layernorm = RMSNorm(self.kv_lora_rank)
        self.kv_a_proj_with_mqa = torch.nn.Linear(self.n_embd, self.kv_lora_rank + self.qk_rope_head_dim, bias=False,)
        self.kv_b_proj = torch.nn.Linear(self.kv_lora_rank, self.n_head * (self.q_head_dim - self.qk_rope_head_dim + 
            self.v_head_dim),bias=False,)
        self.c_proj = torch.nn.Linear(self.n_head * self.v_head_dim, self.n_embd, bias=False)
    ...
    def forward(self, x):
        ...
        kv = (self.kv_b_proj(self.kv_a_layernorm(compressed_kv))
            .view(B, T, self.n_head, self.qk_nope_head_dim + self.v_head_dim)
            .transpose(1, 2))
        ...
        if self.using_groupnorm:
            ...
            y = self.subln(y)
        out = y.transpose(1, 2).contiguous().reshape(B, T, self.n_head * self.v_head_dim).clone()
        out = self.c_proj(out)
        return out
...
class GPTConfig(PretrainedConfig):
    ...
    n_layer : int = 12
    n_head : int = 13 # head dim 128 suggested by @Grad62304977
    n_embd : int = 768
    q_rank: int = 1 # Q rank
    rank: int = 1  # CP rank
